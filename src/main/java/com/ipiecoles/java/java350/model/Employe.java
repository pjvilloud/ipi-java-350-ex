...
public class Employe {
	...
	public void augmenterSalaire(Double pourcentage) throws EmployeException {
		if (pourcentage == null || pourcentage < 0) {
			throw new EmployeException("Le pourcentage ne doit pas être négatif ou null !");
		}
		if (this.getSalaire() == null) {
			throw new EmployeException("La valeur de salaire n'est pas définie !");
		}
		//choix fonctionnel - augmentation max 30%
		if (pourcentage > 0.3) {
			throw new EmployeException("Le pourcentage d'augmentation ne doit pas exceder 30%");
		}
		Double salaireAugmente = this.getSalaire() * (1.0 + pourcentage);
		salaireAugmente = Math.round(salaireAugmente*100d)/100d;
