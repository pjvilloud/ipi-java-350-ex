...
public class Employe {
    ...
    public void augmenterSalaire(double pourcentage) throws SalaireException {
        if (this.salaire == null){
            throw new SalaireException("L'employé n'a pas de salaire");
        }
        double salaireAvant = this.salaire;
        this.salaire = this.salaire + this.salaire*(pourcentage / 100);
        if (salaireAvant > this.salaire){
            throw new SalaireException("Impossible de diminuer le salaire avec la méthode augmenterSalaire");
        }
    }
