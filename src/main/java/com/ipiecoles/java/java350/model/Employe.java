...
public class Employe {
    ...
    public Integer getNbRtt(LocalDate date) {
        ...
        switch (LocalDate.of(date.getYear(), Month.JANUARY, 1).getDayOfWeek()) {
            case THURSDAY:
                if (date.isLeapYear())
                    joursWeekEnd += 1;
                break;
        ...
        return (int) Math.ceil((joursAn - Entreprise.NB_JOURS_MAX_FORFAIT - joursWeekEnd - Entreprise.NB_CONGES_BASE - nbJoursOuvresFeries) * tempsPartiel);
    ...
    public void augmenterSalaire(Double pourcentage) throws EmployeException {
        ...
        if (this.getSalaire() == null || this.getSalaire() <= 0.00d){
            throw new EmployeException("Salaire insuffisant.");
        //Taux invalide
        } else if (pourcentage == null || pourcentage <= 0.00d){
            throw new EmployeException("Taux insuffisant.");
        } else {
